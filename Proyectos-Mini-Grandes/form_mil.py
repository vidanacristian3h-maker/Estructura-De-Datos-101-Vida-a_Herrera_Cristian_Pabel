...
class FrmMil(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("1,000 Datos")
    ...
    def generar_datos(self):
        ...
        messagebox.showinfo("Generar", "✅ Nuevos datos generados correctamente (1,000 datos).")
    ...
    def _actualizar_grafica(self):
        ...
        self.ax.set_xlabel("Tiempo (ms)"); self.ax.set_title(f"Tiempos (1,000 elementos) - {self.tipo.get()}")
        try:
            min_idx = valores.index(min(valores)); max_idx = valores.index(max(valores))
    ...
    def _show_graph_popup(self):
        ...
        ax2.set_xlabel("Tiempo (ms)"); ax2.set_title(f"Tiempos (1,000 elementos) - {self.tipo.get()}")
        try:
            min_idx = valores.index(min(valores)); max_idx = valores.index(max(valores))
            bars[min_idx].set_color("green"); bars[max_idx].set_color("red")
        except Exception:
