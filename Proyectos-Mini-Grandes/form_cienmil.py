...
PAGE_SIZE = 100 # paginación

class FrmCienMil(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("100,000 Datos")
    ...
    def generar_datos(self):
        ...
        messagebox.showinfo("Generar", "✅ Nuevos datos generados correctamente (100,000 datos).")
    ...
    def _ejecutar(self):
        ...
        resultados = []
        self.after(0, lambda: self.status_lbl.config(text="Ejecutando algoritmos rápidos..."))

        for nombre, funcion in metodos:
            if funcion == "PESADO ⚠️":
                resultados.append((nombre, "PESADO ⚠️"))
    ...
    def _mostrar_tiempos_parciales(self, parcial):
        ...
        for n,t in ps:
            self.tree_times.insert("", "end", values=(n, str(t)))
    ...
    def _actualizar_grafica(self):
        ...
        nombres = [t[0] for t in self.tiempos if isinstance(t[1], (int,float))]
        valores = [float(t[1]) for t in self.tiempos if isinstance(t[1], (int,float))]
        self.ax.clear(); bars = self.ax.barh(nombres, valores); self.ax.invert_yaxis()
        self.ax.set_xlabel("Tiempo (ms)"); self.ax.set_title(f"Tiempos (100,000 elementos) - {self.tipo.get()}")
        try:
            min_idx = valores.index(min(valores)); max_idx = valores.index(max(valores))
